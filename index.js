...
// User Chats Schema for storing a list of a user's conversations
...
const chatSchema = new mongoose.Schema({
  ...
  model: { type: String },
  createdAt: { type: Date, default: Date.now },
  // All of the messages in the chat
  messages: [
    {
      author: {
    ...
    // This structure is determined by the flutter plugin that knows how to show chat messages
...
// Assume everything is JSON
app.use(express.json());

// Update or insert
// This will EITHER update the title of an existing chat OR create a new one with this label
async function upsertChat(userId, chatId, label, createdAt) {
  try {
    console.log(
      `🔍 Upserting chat for userId=${userId}, chatId=${chatId}, label=${label}`
    );

    // Step 1: Try to update only the label (title) of an existing chat
    const result = await UserChats.findOneAndUpdate(
      { userId, "chats.chatId": chatId }, // Find by userId and chatId within chats list
    ...
    if (!result) {
      ...
      const insertResult = await UserChats.findOneAndUpdate(
        ...
        {
          $push: {
            // ✅ Using $push ensures chatId is inserted correctly
            chats: {
              chatId: chatId,
              label: label,
              createdAt: createdAt,
...
async function upsertChatMessages(userId, chatId, model, createdAt, messages) {
  try {
    console.log(
      `🔍 Upserting messages for userId=${userId}, chatId=${chatId}, model=${model}`
    );
...
// IMPROVEMENT: adopt normal RESTful structure (POST, GET, PUT, DELETE)
...
app.post("/chat/save-messages", async (req, res) => {
